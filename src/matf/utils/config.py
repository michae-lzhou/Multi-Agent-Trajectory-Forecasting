...
def make_run_name(cfg):
    if cfg.model.type == "lstm":
        ...
        return (f"transformer_h{cfg.model.hidden_size}"
                f"_heads{cfg.model.num_heads}"
                f"_res{cfg.model.use_residual}"
                f"_norm{cfg.model.layer_norm}"
                f"_cell_state{cfg.model.cell_state}")
